...
matrix.expand <- function(the.matrix, new.rows){
  ...
  matrix.expand <- rbind(the.matrix,new.rows.matrix)
  matrix.expand <- matrix.expand[order(as.numeric(rownames(matrix.expand))),]
  matrix.expand <- zoo::na.locf(matrix.expand)
  matrix.expand[is.na(matrix.expand)] <- 0
